...
import { useMachineHistory, useMachinePhoto } from "../../hooks/useStorage";
...
export default function MachineDetailScreen() {
  ...
  const takePhoto = async () => {
    ...
    if (status !== "granted") return;
    const r = await ImagePicker.launchCameraAsync({ allowsEditing: true, aspect: [16, 9], quality: 0.7 });
    if (!r.canceled && r.assets[0]) updatePhoto(r.assets[0].uri);
  };
  ...
  const handlePhotoPress = () => {
    if (photo) {
      Alert.alert("Foto", "", [
        { text: "Trocar", onPress: () => Alert.alert("De onde?", "", [
  ...
  return (
    ...
    >
      ...
      >
        {photo ? (
          ...
          <View style={{ alignItems: "center", gap: 6 }}>
            ...
            <Text style={{ color: t.textDim, fontSize: 12, fontWeight: "600" }}>adicionar foto</Text>
          </View>
        )}
      </TouchableOpacity>
      ...
      {machine.description && (
        <Text style={{ color: t.textMuted, fontSize: 14, lineHeight: 20, marginBottom: 16 }}>
          {machine.description}
        </Text>
      )}
      ...
      {history.length === 0 ? (
        <Text style={{ color: t.textDim, textAlign: "center", marginTop: 24, fontSize: 14 }}>
                ...
                <View style={{ flexDirection: "row", gap: 14 }}>
                  {item.sets.map((w, i) => (
                    <Text key={i} style={{ color: t.accent, fontSize: 14, fontWeight: "700" }}>
                      {w}<Text style={{ color: t.textMuted, fontSize: 11 }}>kg</Text>
                    </Text>
                  ))}
                </View>
              </View>
