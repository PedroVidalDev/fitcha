...
import { CreditCheckoutResponse, CreditSummaryResponse, UserPayment } from "../@types/credit";
...
export function useCreditCheckout(params: UseCreditCheckoutParams = {}) {
    ...
    const generateCheckout = useCallback(async () => {
        ...
        try {
            setErrorMessage(null);
            const response = await createCreditCheckout(parsedQuantity, cleanDocument);
            await syncSummary(response);
            setStep(3);
        } catch (error) {
            setErrorMessage(
                error instanceof Error ? error.message : t("creditCheckout.error.generatePix"),
            );
        } finally {
