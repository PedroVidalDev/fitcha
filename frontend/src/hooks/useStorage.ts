...
async function getData(): Promise<AppData> {
  ...
  if (raw) {
    ...
    cache = { machines: {}, days: { 0: [], 1: [], 2: [], 3: [], 4: [], 5: [], 6: [] }, history: {} };
  }
  return cache!;
}
...
export function useWeek() {
  ...
  const addMachineToDay = useCallback(async (
    dayIndex: number, name: string, categoryKey: MachineCategoryKey, description?: string
  ) => {
    const data = await getData();
    const id = uid();
  ...
  const removeMachineFromDay = useCallback(async (dayIndex: number, machineId: string) => {
    const data = await getData();
    data.days[dayIndex] = (data.days[dayIndex] ?? []).filter((id) => id !== machineId);
    // se a máquina não está em nenhum outro dia, remove ela e o histórico
